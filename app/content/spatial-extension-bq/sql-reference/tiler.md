...
Creates a simple tileset. It differs from `tiler.CREATE_SIMPLE_TILESET` in that the procedure performs a previous analysis in order to automatically find the right options for the tileset. It is done by extracting all the properties to be included within the tileset and sampling the data in order to avoid BigQuery limitations. Therefore, only `source_table` and `target_table` are mandatory and `options` can be set to `NULL`.
