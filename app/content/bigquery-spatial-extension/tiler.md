...
2. `tiler.CreatePointAggregationTileset`
...
CALL bqcarto.tiler.CreateSimpleTileset(
...
##### NYC trees (CreatePointAggregationTileset)
...
```sql
--Use bqcartoeu if using the Spatial Extension in Europe's region
CALL bqcarto.tiler.CreatePointAggregationTileset(
--SQL to use as the source (uses geom as the name for the geography)
