...
{{% bannerNote type="warning" title="warning"%}}
**We recommend using this function only with an input of up to 10 records. In order to geocode larger sets of locations, we strongly recommend using the [GEOCODE_TABLE](/analytics-toolbox-redshift/sql-reference/lds/#geocode_table) procedure. Likewise, in order to materialize the results in a table.**
